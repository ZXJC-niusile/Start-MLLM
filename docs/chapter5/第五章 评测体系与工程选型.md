...
set DATASET_PATH=sample_eval_dataset.jsonl
set OUTPUT_PATH=eval_results.jsonl
python eval_vlm_dataset.py
```

Linux / macOS 可将上述 `set` 行换成（示例）：
...
也可用仓库内 **`docs/chapter5/code/summarize_eval_results.py`**（标准库）直接汇总：单文件会打印错误率、粗粒度参考命中率、耗时均值/中位数，以及按 `tags` 分桶；多文件对比可加 `--compare` 与 `--markdown`；可加 **`--write 路径.md`** 把输出写成 UTF-8 文件（终端编码异常时尤便）。尚未调用真实接口时，可用同目录下的 **`sample_eval_results_output.jsonl`**（示例输出格式）试跑汇总脚本。详见 [研究向补充：微调入口、数据衔接与评测汇总](../研究向补充-微调与实验记录.md)。
...
评测章节建议配一张“五维评测矩阵图”（感知、OCR、推理、幻觉、工程成本）和一张“模型选型看板图”（准确性、延迟、显存、适配场景）。
